...
public typealias DownloadTaskOperationResult = (url: URL?, urlResponse: URLResponse?, error: Error?)

open class DownloadTaskOperation: NetworkOperation {
    // MARK: - Properties

    private let requestInterceptors: [RequestInterceptorProtocol]?
    private let responseInterceptors: [DownloadTaskResponseInterceptorProtocol]?
    public private(set) var result: DownloadTaskOperationResult?

    private let urlSession: URLSession
    private var request: URLRequest
    private var task: URLSessionDownloadTask?
    private lazy var taskFactory: (URLSession, URLRequest, @escaping (Bool) -> Void) -> URLSessionDownloadTask? = { urlSession, request, completion in
        return urlSession.downloadTask(
            with: request,
            completionHandler: { [weak self] localURL, response, error in
                ...
                self.result = DownloadTaskOperationResult(url: localURL, urlResponse: response, error: error)
            }
        )
    }
                ...
                requestInterceptor.adapt(
                    ...
                    completion: { [weak self] resultType in
                        ...
                        case .error(let error):
                            ...
                            self?.result = DownloadTaskOperationResult(url: nil, urlResponse: nil, error: error)
                            completion(false)
                            return
    ...
    open override func performResponseInterceptors(completion: @escaping (Bool) -> Void) {
        ...
        /// Call all interceptor's adapt function sequentially
        var closures = [(Int) -> Void]()
        let closureFactory: (DownloadTaskResponseInterceptorProtocol) -> ((Int) -> Void) = { responseInterceptor in
            let closure: (Int) -> Void = { [weak self] index in
                guard let self = self else { return }
